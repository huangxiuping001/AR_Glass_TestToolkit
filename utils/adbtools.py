...
class AdbTools(object):
    ...
    def __connection_devices(self):
        ...
        if self.__device_id == "":
            return
        self.__device_id = "%s" % self.__device_id
    ...
    def adb(self, args):
        ...
        :return:
        """
        cmd = "adb -s %s %s" % (self.__device_id, str(args))
    ...
    def shell(self, args):
        ...
        """
        if self.__device_id == '':
            cmd = "adb shell " + str(args)
        else:
            cmd = "adb -s " + self.__device_id + " shell " + str(args)
        return os.popen(cmd)

    def shell_pi(self, args):
        """
        执行adb shell命令
        :param args:参数
        :return: pi
        ...
        pi = subprocess.Popen(cmd, shell = True, close_fds=True, stdout = subprocess.PIPE)
    ...
    def __get_process(self, package_name):
        ...
        """
        if package_name == "":
            print("packageName can not null!")
            return ''

        if self.__system == "Windows":
            pid_command = self.shell_pi("ps | %s %s" % (self.__find, package_name)).stdout.read()
        else:
    ...
    def get_pid(self, package_name):
        ...
        pid_command = str(pid_command, encoding='utf8')

        if pid_command == '':
            print("The process doesn't exist.")
            return pid_command

        print("------------" + str(pid_command))
        req = re.compile(r"\d+")
